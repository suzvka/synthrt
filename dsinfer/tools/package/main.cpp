...
static int installPackage(
    const fs::path &packerPath, const fs::path &outputDir, const fs::path &checkFilePath = "",
    ...
    if (!checkResult) {
        throw std::runtime_error("Package validation failed for '" + packerPath.string() +
...
// @param checkFunction: Custom file check rule: the file content will be passed to this function,
// and an empty data block will be returned if the target file is abnormal
// @param uninstallCallback: Callback triggered before uninstallation, return true to continue
// uninstallation
//
//------------------------------------------------------------------------------
static int uninstallPackage(
    const fs::path &installedDir, const fs::path &checkFilePath = "",
    ContentCheck checkFunction = [](const std::vector<std::byte> &) { return true; },
    UninstallCallBack uninstallCallback = [](const std::wstring &) { return true; }) {
    if (!uninstallCallback(installedDir.stem().filename())) {
        return -1;
    }
    ...
    auto checkResult = check.check();
    if (!checkResult) {
        throw std::runtime_error("Uninstallation check failed for directory '" +
                                 installedDir.string() + "': " + checkResult.error().message());
    }

    std::error_code ec;
    fs::remove_all(installedDir, ec);
    if (ec) {
        throw std::runtime_error("Failed to remove directory '" + installedDir.string() +
                                 "': " + ec.message());
    }
